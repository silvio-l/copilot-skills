...
description: "MUST USE for ANY onboarding, signup-flow UX, first-run UX, activation, user adoption, getting-started flow, product tour, walkthrough, empty-state-to-first-value, permission timing during onboarding, progressive disclosure in first-run flows, or paywall-in-onboarding work. Invoke this skill IMMEDIATELY whenever the user asks to create, review, improve, audit, simplify, optimize, redesign, or measure onboarding for an app, website, SaaS product, desktop app, or digital workflow. Also trigger when the request mentions activation, time-to-value, aha moment, first session, new user friction, onboarding completion, trial conversion, or reducing drop-off for new users, even if the word 'onboarding' is not used. Do NOT use this skill as the primary skill for generic signup/auth implementation, generic permission/privacy reviews, generic empty states, generic progressive disclosure, or non-onboarding UI/copy work unless they are clearly tied to new-user onboarding or first-run activation. Also triggers on German: 'Onboarding', 'Nutzer onboarding', 'Registrierungsflow', 'Signup Flow', 'erste Nutzung', 'First Run', 'Aktivierung', 'Time to Value', 'Aha Moment', 'Drop-off', 'Permission Prompt im Onboarding', 'Paywall im Onboarding', 'Benutzer einfuehren', 'Getting Started'. This skill COMPLEMENTS premium-ui-ux and ui-copy-localization and REPLACES ad-hoc onboarding design/review - never design or review onboarding without it."
---
